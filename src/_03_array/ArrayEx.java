...
public class ArrayEx {
  ...
  public static void main(String[] args) {
    ...
    // - 배열 변수는 "참조 변수". 배열도 객체이므로 힙 영역에 생성되고, 배열 변수는 힙 영역의 주소를 저장함
    // null 저장 가능 (null 로 초기화)
    // ex. 타입[] 변수 = null;
    // => 배열 변수가 null 값을 가진 상태에서는 변수[index]로 값을 읽거나 저장하게 되면 NullPointerException 발생
    String[] temp = null;
    ...
    for (int i = 0; i < threeDimensionArr.length; i++) {
      for (int j = 0; j < threeDimensionArr[i].length; j++) {
        //                System.out.print(threeDimensionArr[i][j] + " ");
    ...
    // 배열 복사
    // - 배열은 크기가 고정
    // -> 더 많은 저장 공간 필요하다면 더 큰 길이의 배열을 새로 만들어 기존 배열 복사
    ...
    // ver 2. arraycopy() 사용
    // : System.arraycopy(Object src, int srcPos, Object dest, int destPost, int lenght);
    // - Object src: 원본 배열
    ...
    System.out.println(
      "filled Array (before): " + Arrays.toString(filledArray)
    );
    ...
    System.out.println("Arrays Equal (2 vs 3): " + arraysEqual2); // false

    // == 비교
    System.out.println("Arrays Equal (1 vs 2): " + (array1 == array2)); // false
    System.out.println("Arrays Equal (2 vs 3): " + (array1 == array3)); // false
    // 참조하고 있어서 제대로 비교할 수 없음
    ...
    // binarySearch(arr, value) 메소드 (단, 배열은 정렬된 상태여야 함)
